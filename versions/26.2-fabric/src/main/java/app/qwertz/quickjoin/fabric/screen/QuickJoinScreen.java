...
public class QuickJoinScreen extends Screen {
    ...
    protected void init() {
        ...
        if ("QuickJoinGui".equals(guiName)) {
            ...
            if (QuickJoinConfig.enableFavorites()) {
                addRenderableWidget(Button.builder(Component.literal("§6§lFavorites"), btn -> Minecraft.getInstance().gui.setScreen(new FavoritesScreen()))
                        .bounds(nextX, topY, 80, 20)
                        .build());
                nextX -= spacing;
            }

            addRenderableWidget(Button.builder(Component.literal("§a§lSearch"), btn -> Minecraft.getInstance().gui.setScreen(new SearchScreen()))
                    .bounds(nextX, topY, 80, 20)
                    .build());
            nextX -= spacing;
            ...
            addRenderableWidget(Button.builder(Component.literal("§b§lConfig"), btn -> {
                        if (Minecraft.getInstance() != null) {
                            Minecraft.getInstance().gui.setScreen(QuickJoinConfig.createConfigScreen(this));
                        }
                    })
                    .bounds(nextX, topY, 80, 20)
                    .build());
    ...
    private void handleButtonPress(int id) {
        if (id == 7000) {
            Minecraft.getInstance().gui.setScreen(new FavoritesScreen());
            return;
        }
        if (id == 7001) {
            Minecraft.getInstance().gui.setScreen(new SearchScreen());
            return;
        }
            ...
            for (Button widget : entryButtons) {
                if (widget.isMouseOver(mouseX, mouseY)) {
                    ...
                    if (QuickJoinConfig.debugMode() && Minecraft.getInstance().player != null) {
                        if (Minecraft.getInstance().gui != null && Minecraft.getInstance().gui.hud.getChat() != null) {
                            Minecraft.getInstance().gui.hud.getChat().addClientSystemMessage(Component.literal(
                                    "Favorites " + (added ? "added" : "removed") + ": " + guiName + "#" + id
                            ));
                        }
                    }
                    init();
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        Identifier icon = Identifier.fromNamespaceAndPath(QuickJoinFabric.MOD_ID, "icon" + gui.icon.sheet + ".png");
        ...
        context.centeredText(font, Component.literal(gui.name), this.width / 2, titleY, 0xFFFFFFFF);
            }
